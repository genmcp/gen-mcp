...
2. Run the HTTP-based integration:
   ```bash
   genmcp run -t examples/ollama/ollama-http.yaml -s examples/ollama/ollama-server.yaml
   ```
...
2. Run the CLI-based integration:
   ```bash
   genmcp run -t examples/ollama/ollama-cli.yaml -s examples/ollama/ollama-cli-server.yaml
   ```
